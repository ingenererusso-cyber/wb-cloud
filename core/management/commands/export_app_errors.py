...
from core.models import AppErrorLog
...
class Command(BaseCommand):
    help = "Экспорт логов ошибок приложения (AppErrorLog) в JSON/CSV."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--source", type=str, default=None, help="Фильтр по source")
        parser.add_argument("--level", type=str, default=None, help="Фильтр по level")
    ...
    def handle(self, *args, **options):
        ...
        qs = AppErrorLog.objects.select_related("user", "seller").order_by("-created_at")
        ...
        if not out_path_raw:
            ...
            out_path_raw = f"exports/app_errors_{stamp}.{export_format}"
        ...
        self.stdout.write(self.style.SUCCESS(f"Экспортировано ошибок: {len(rows)}"))
